...
@class AZAPIBaseManager;

// 在调用成功之后的params字典里面，用这个key可以取出requestID
static NSString * const kAZAPIBaseManagerRequestID = @"kAZAPIBaseManagerRequestID";


/*************************************************************************************************/
/*                               AZAPIManagerAPICallBackDelegate                                 */
/*************************************************************************************************/
//api回调
@protocol AZAPIManagerAPICallBackDelegate <NSObject>
@required
...
/*************************************************************************************************/
/*                               AZAPIManagerCallbackDataReformer                                */
/*************************************************************************************************/
//负责重新组装API数据的对象
/*
 重组数据的场景：
...
@protocol AZAPIManagerCallbackDataReformer <NSObject>
@required
/*
 ...
 */
- (id)manager:(AZAPIBaseManager *)manager reformData:(NSDictionary *)data;
@end

/*************************************************************************************************/
/*                                     AZAPIManagerValidator                                     */
/*************************************************************************************************/
 ...
 */
@protocol AZAPIManagerValidator <NSObject>
@required
/*
...
/*************************************************************************************************/
/*                                AZAPIManagerParamSourceDelegate                                */
/*************************************************************************************************/
//让manager能够获取调用API所需要的数据
@protocol AZAPIManagerParamSourceDelegate <NSObject>
@required
 ...
 */
typedef NS_ENUM (NSUInteger, AZAPIManagerErrorType){
    AZAPIManagerErrorTypeDefault,       // 没有产生过API请求，这个是manager的默认状态。
    AZAPIManagerErrorTypeSuccess,       // API请求成功且返回数据正确，此时manager的数据是可以直接拿来使用的。
    AZAPIManagerErrorTypeNoContent,     // API请求成功但返回数据不正确。如果回调数据验证函数返回值为NO，manager的状态就会是这个。
    AZAPIManagerErrorTypeParamsError,   // 参数错误，此时manager不会调用API，因为参数验证是在调用API之前做的。
    AZAPIManagerErrorTypeTimeout,       // 请求超时。AZAPIProxy设置的是20秒超时，具体超时时间的设置请自己去看AZAPIProxy的相关代码。
    AZAPIManagerErrorTypeNoNetWork      // 网络不通。在调用API之前会判断一下当前网络是否通畅，这个也是在调用API之前验证的，和上面超时的状态是有区别的。
};

typedef NS_ENUM (NSUInteger, AZAPIManagerRequestType){
    AZAPIManagerRequestTypeGet,
    AZAPIManagerRequestTypePost,
    AZAPIManagerRequestTypeBodyPost,
    AZAPIManagerRequestTypeRestGet,
    AZAPIManagerRequestTypeRestPost
};
...
/*************************************************************************************************/
/*
 AZAPIBaseManager的派生类必须符合这些protocal
 */
@protocol AZAPIManager <NSObject>
...
/*************************************************************************************************/
/*                                    AZAPIManagerInterceptor                                    */
/*************************************************************************************************/
/*
 AZAPIBaseManager的派生类必须符合这些protocal
 */
@protocol AZAPIManagerInterceptor <NSObject>

@optional
- (void)manager:(AZAPIBaseManager *)manager beforePerformSuccessWithResponse:(AZURLResponse *)response;
- (void)manager:(AZAPIBaseManager *)manager afterPerformSuccessWithResponse:(AZURLResponse *)response;

- (void)manager:(AZAPIBaseManager *)manager beforePerformFailWithResponse:(AZURLResponse *)response;
- (void)manager:(AZAPIBaseManager *)manager afterPerformFailWithResponse:(AZURLResponse *)response;

- (BOOL)manager:(AZAPIBaseManager *)manager shouldCallAPIWithParams:(NSDictionary *)params;
- (void)manager:(AZAPIBaseManager *)manager afterCallingAPIWithParams:(NSDictionary *)params;
...
/*                                       AZAPIBaseManager                                        */
/*************************************************************************************************/
@interface AZAPIBaseManager : NSObject

@property (nonatomic, weak) id<AZAPIManagerAPICallBackDelegate> delegate;
@property (nonatomic, weak) id<AZAPIManagerParamSourceDelegate> paramSource;
@property (nonatomic, weak) id<AZAPIManagerValidator> validator;
@property (nonatomic, weak) NSObject<AZAPIManager> *child; //里面会调用到NSObject的方法，所以这里不用id
@property (nonatomic, weak) id<AZAPIManagerInterceptor> interceptor;
...
@property (nonatomic, readonly) AZAPIManagerErrorType errorType;
